...
# Contains a very basic bulk feature to generate 2 weeks worth of multitrack templates for every single day of the week.
...
def generate_template_folders(month, year, day, num_parents, num_subfolders, use_template, subfolder_prefix, output_path=None):
    ...
    for i in range(num_parents):
        ...
        root_folder = os.path.join(output_path, f"{day_name} Prepped Multitracks")
        os.makedirs(root_folder, exist_ok=True)

        # Create a subfolder for each day's parent
        current_root_folder = os.path.join(root_folder, f"ET {year}-{month:02d}-{current_day:02d} {day_name}")
        os.makedirs(current_root_folder)

        # Generate subfolders
        for j in range(1, num_subfolders + 1):
            subfolder_name = f"{subfolder_prefix}{j:02d}" if subfolder_prefix else f"Session_{j:02d}"
            subfolder = os.path.join(current_root_folder, subfolder_name)
        ...
        readme_path = os.path.join(current_root_folder, "README.md")
        ...
        open_all_projects_path = os.path.join(current_root_folder, "OpenAllProjects.RPL")
...
try:
    bulk_flag = input("Do you want to generate 10 multitrack folders for all days of the week at once? (yes or no)").lower()

    if bulk_flag == "yes":
        for day_number in range(1, 8):
            print(f"Generating folders for Day {day_number}...")
            # Assuming a fixed month and year for bulk generation, you can modify this accordingly
            generate_template_folders(month=2, year=2024, day=day_number, num_parents=10, num_subfolders=10, use_template=True,
                                    subfolder_prefix="", output_path=None)
    else:
        # Get user input

...
def generate_template_folders(month, year, day, num_parents, num_subfolders, use_template, subfolder_prefix):
    ...
    for i in range(num_parents):
        ...
        for j in range(1, num_subfolders + 1):
            ...
            # Copy template folder to each subfolder if specified
            if use_template:
                template_folder = "Template"
                if os.path.exists(template_folder):
                    shutil.copytree(template_folder, os.path.join(subfolder, "Template"))

            # Create README.md
            with open(os.path.join(subfolder, "README.md"), "w") as readme:
                readme.write(f"# {subfolder}\n\nGenerated by etgen.py")
